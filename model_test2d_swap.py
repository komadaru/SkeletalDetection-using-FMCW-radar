...
def create_dataset(inputsxy, inputsyz, targets, sequence_size=1):
    dataset = []

    for i in range(len(inputsxy) - sequence_size + 1):
        input_sequence = (inputsxy[i:i + sequence_size], inputsyz[i:i + sequence_size])
...
test_dir = "C:/Users/kodam/PycharmProjects/mmModels/Datasets/ScannedOneRadar3"
...
data = TensorDataset(test_inputsxy, test_inputsyz, test_targets)
dataset = create_dataset(data.tensors[0], data.tensors[1], data.tensors[2])
num_iterations_per_epoch = len(dataset) // batch_size

# 学習済みモデルの読み込み
model = PointNetmmPoseCNN(num_points, num_labels).cuda()
model.load_state_dict(torch.load("C:/Users/kodam/PycharmProjects/mmModels/Models/OneRadar/saved_model_71.pth"))
...
for trial in range(5):
    for iteration in range(num_iterations_per_epoch):
        ...
        #model.eval()
        ts_output = model(ts_inputsxy, ts_inputsyz)
