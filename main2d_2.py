...
print(len(inputszx))
print(len(inputszx[0]))
print(len(inputszx[0][0]))
inputszx = np.array(inputszx)
...
inputszx = torch.from_numpy(inputszx)
targets = torch.from_numpy(targets)
inputsxy = inputsxy.to(torch.float32)
inputsyz = inputsyz.to(torch.float32)
inputszx = inputszx.to(torch.float32)
targets = targets.to(torch.float32)

def create_dataset(inputsxy, inputsyz, inputszx, targets, sequence_size=1, random_sampling=True):
    dataset = []
    ...
    if random_sampling:
        ...
        for i in range(len(inputsxy) - sequence_size + 1):
            ...
            input_sequence_zx = inputszx[start_idx:start_idx + sequence_size]

            input_sequence = (input_sequence_xy, input_sequence_yz, input_sequence_zx)
...
def main(input_dataxy, input_datayz, input_datazx, target, num_epochs=epochs, batch_size=1, sequence_size=1):
    ...
    data = TensorDataset(input_dataxy, input_datayz, input_datazx, target)
    ...
    for epoch in range(1, num_epochs+1):
        ...
        for iteration in range(num_iterations_per_epoch):
            ...
            batch_inputszx = torch.stack([torch.Tensor(item[0][2]) for item in batch_data]).squeeze(dim=0).cuda()
            #print(batch_inputs)
            ...
            pointnet.zero_grad()
            output = pointnet(batch_inputsxy, batch_inputsyz, batch_inputszx)
        ...
        with torch.no_grad():
            for val_item in val_d:
                ...
                val_inputszx = torch.Tensor(val_item[0][2]).unsqueeze(0).cuda()
                val_targets = torch.Tensor(val_item[1]).unsqueeze(0).cuda()

                val_output = pointnet(val_inputsxy, val_inputsyz, val_inputszx)
        ...
        # モデルの保存
        output_folder = "C:/Users/kodam/PycharmProjects/mmModels/Models/TwoRadars_tilt_mmPose_3dir/"
...
main(inputsxy, inputsyz, inputszx, targets)