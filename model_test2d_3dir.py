...
def create_dataset(inputsxy, inputsyz, inputszx, targets, sequence_size=1):
    dataset = []

    for i in range(len(inputsxy) - sequence_size + 1):
        input_sequence = (inputsxy[i:i + sequence_size], inputsyz[i:i + sequence_size], inputszx[i:i + sequence_size])
...
for i in range(num_flames+1):
    ...
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as json_file:
            ...
            test_inputsxy.append(batch_list)
...
for i in range(num_flames+1):
    ...
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as json_file:
            ...
            batch_list = []
            for x in range(data["NumPoints"]):
                batch_list.append([data["PointCloud"][2][x], data["PointCloud"][0][x], data["PointCloud"][4][x]])
            ...
            test_inputszx.append(batch_list)

print(len(test_inputsxy))
print(len(test_inputsxy[0]))
print(len(test_inputsxy[0][0]))
test_inputsxy = np.array(test_inputsxy)
print(len(test_inputsyz))
print(len(test_inputsyz[0]))
print(len(test_inputsyz[0][0]))
test_inputsyz = np.array(test_inputsyz)
print(len(test_inputszx))
print(len(test_inputszx[0]))
print(len(test_inputszx[0][0]))
test_inputszx = np.array(test_inputszx)
...
test_inputsxy = torch.from_numpy(test_inputsxy)
test_inputsyz = torch.from_numpy(test_inputsyz)
test_inputszx = torch.from_numpy(test_inputszx)
test_targets = torch.from_numpy(test_targets)
test_inputsxy = test_inputsxy.to(torch.float32)
test_inputsyz = test_inputsyz.to(torch.float32)
test_inputszx = test_inputszx.to(torch.float32)
test_targets = test_targets.to(torch.float32)

data = TensorDataset(test_inputsxy, test_inputsyz, test_inputszx, test_targets)
...
# 学習済みモデルの読み込み
model = PointNetmmPoseCNN(num_points, num_labels).cuda()
model.load_state_dict(torch.load("C:/Users/kodam/PycharmProjects/mmModels/Models/TwoRadars_tilt_mmPose_3dir/saved_model_46.pth"))
...
for iteration in range(num_iterations_per_epoch):
    ...
    ts_inputsxy = torch.stack([torch.Tensor(item[0][0]) for item in data]).squeeze(dim=0).cuda()
    ts_inputsyz = torch.stack([torch.Tensor(item[0][1]) for item in data]).squeeze(dim=0).cuda()
    ts_inputszx = torch.stack([torch.Tensor(item[0][2]) for item in data]).squeeze(dim=0).cuda()
    #print(ts_inputs)
    middle_index = len(data) // 2
    ts_targets = torch.Tensor(data[middle_index][1]).unsqueeze(0).cuda()

    #model.eval()
    ts_output = model(ts_inputsxy, ts_inputsyz, ts_inputszx)
