    ...
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as json_file:
            ...
            for x in range(data["NumPoints"]):
                # Extract x, y, and snr values
                x_val = data["PointCloud"][0][x]
                y_val = data["PointCloud"][1][x]
                snr_val = data["PointCloud"][4][x]

                # Normalize snr value to the range [0, 255]
                snr_normalized = float(255 * (snr_val - np.min(data["PointCloud"][4])) / (
                            np.max(data["PointCloud"][4]) - np.min(data["PointCloud"][4])))

                x_normalized = float(255 * (x_val - x_min) / (x_max - x_min))

                y_normalized = float(255 * (y_val - y_min) / (y_max - y_min))

                # Assign [x, y, normalized_snr] to the corresponding location in batch_list
                batch_list.append([x_val, y_val, snr_val])

            # Check if NumPoints is less than num_points
            if len(batch_list) < im_size**2:
                # Fill the remaining points with zeros
                batch_list += [[0, 0, 0]] * (im_size**2 - len(batch_list))

            inputsxy.append([batch_list[i:i+im_size] for i in range(0, len(batch_list), im_size)])
    ...
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as json_file:
            ...
            for x in range(data["NumPoints"]):
                ...
                z_normalized = float(255 * (z_val - z_min) / (z_max - z_min))

                # Assign [x, y, normalized_snr] to the corresponding location in batch_list
                batch_list.append([y_val, z_val, snr_val])
            ...
            inputsyz.append([batch_list[i:i + im_size] for i in range(0, len(batch_list), im_size)])
...
inputsxy = inputsxy.permute(0, 3, 1, 2)
inputsyz = inputsyz.to(torch.float32)
inputsyz = inputsyz.permute(0, 3, 1, 2)
...
def main(input_dataxy, input_datayz, target, num_epochs=epochs, batch_size=1, sequence_size=1):
    pointnet = mmPose(im_size, 3, num_labels).cuda()
    ...
    for epoch in range(1, num_epochs+1):
        ...
        with torch.no_grad():
            for val_item in val_d:
                val_inputsxy = torch.Tensor(val_item[0][0]).cuda()
                val_inputsyz = torch.Tensor(val_item[0][1]).cuda()
        ...
        # モデルの保存
        output_folder = "C:/Users/kodam/PycharmProjects/mmModels/Models/TwoRadars_tilt_mmPose_pixel/"
