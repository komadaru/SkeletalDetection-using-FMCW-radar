...
dir1 = "C:/Users/kodam/PycharmProjects/mmModels/DataMake/RawData/mmData1223_rm/TwoRadars_notilt/upper_fixed/"
dir2 = "C:/Users/kodam/PycharmProjects/mmModels/DataMake/RawData/mmData1223_rm/TwoRadars_notilt/lower_fixed/"
skedir = "C:/Users/kodam/PycharmProjects/mmModels/DataMake/RawData/Data1223/TwoRadars_notilt/20231223/"
savedir = "C:/Users/kodam/PycharmProjects/mmModels/Datasets/TwoRadars_notilt/"

####################################### 1 ######################################
# フレームを決定し、リストへ格納
start_time = datetime(2023, 12, 23, 15, 15, 51) #変数
end_time = datetime(2023, 12, 23, 15, 17, 33) #変数
...
for i in range(start_index2, end_index2 + 1):
    ...
    if os.path.exists(file_path):
        with open(file_path, "r", encoding="utf-8") as json_file:
            ...
            # original_time = data["Now_time"]
            # removed_time = original_time[:-6]
            # data2_times.append([f"{i}.json", datetime.strptime(removed_time, '%Y-%m-%d %H:%M:%S.%f')])
            #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

            #タイムゾーン情報を削除しない場合^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            data2_times.append([f"{i}.json", datetime.strptime(data["Now_time"], '%Y-%m-%d %H:%M:%S.%f')])
            #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
frame_and_json_list2 = []
for time_idx in range(len(time_list)-1):
    st_time = time_list[time_idx]
    ed_time = time_list[time_idx + 1]
    frame_and_json_list2.append([st_time, ed_time])

    ts = []
    for dtime in data2_times:
        if st_time <= dtime[1] < ed_time:
            ts.append(dtime[0])
    if ts:
        frame_and_json_list2[time_idx].append(ts)
    else:
        frame_and_json_list2[time_idx].append('blank')
...
for i in range(len(frame_and_json_list1)):
    ...
    if not frame_and_json_list2[i][2] == 'blank':
        for jsons in frame_and_json_list2[i][2]:
            file_path2 = dir2 + jsons
            with open(file_path2, 'r', encoding='utf8') as json_file:
                data = json.load(json_file)
                if data["NumPoints"] > 0:
                    for pidx in range(data["NumPoints"]):
                        PointCloud[0].append((data["PointCloud"][0][pidx])*1000)
    ...
    if not (list1_is_blank and list2_is_blank) and not kp_is_blank:

        NRKeyPoints = []
        for j in range(0, len(KeyPoints[0]), 3):
            x, y, z = KeyPoints[0][j:j + 3]
        ...
        now_time_str = frame_and_json_list1[i][0].strftime('%Y-%m-%d %H:%M:%S.%f')
        small_data = {
